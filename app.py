...
st.set_page_config(page_title="رادار المشتريات العالمي - إعداد أبو نايف", page_icon="🌍", layout="centered")

# --- 2. ستايل CSS متطور للجوال ودعم كامل للغة العربية ---
st.markdown("""
    <style>
    ...
    /* تحسين الأزرار لتناسب شاشة الجوال */
    .stLinkButton a {
...
MARKET_LOGIC = {
    "الصين 🇨🇳": {
        ...
        "إلكترونيات وأجهزة": [
            ("AliExpress Tech", "https://www.aliexpress.com/category/44/consumer-electronics.html?SearchText="),
    ...
    "تركيا 🇹🇷": {
        "أثاث وديكور": [
            ("Trendyol Home", "https://www.trendyol.com/ev--yasama-x-c104603?q="),
            ("Hepsiburada Home", "https://www.hepsiburada.com/ev-yasam-ofis-kirtasiye-c-2147483644?q="),
...
FIELDS = [
    "بناء وإعمار", "أثاث وديكور", "إلكترونيات وأجهزة", "سيارات وقطع غيار", 
    "ملابس وأزياء", "مواد غذائية وجملة", "معدات صناعية وآلات", 
    "عقارات", "أدوات صحية وسباكة", "عام / أخرى"
]
...
if item_ar:
    with st.spinner('⏳ جاري الترجمة والبحث في المصادر...'):
        try:
            ...
            translated_text = translator.translate(item_ar, dest=LANGUAGES[target_lang]).text
            
            st.info(f"🔍 يتم البحث الآن عن: **{translated_text}**")
            
            country_data = MARKET_LOGIC.get(target_country, {})
            # جلب المواقع حسب التخصص، أو الرجوع للعام إذا لم يوجد تخصص محدد للدولة
            ...
            if top_sites:
                ...
                st.warning("لم يتم العثور على مواقع مخصصة لهذا المجال في هذه الدولة، جرب 'عام'.")
                
        except Exception:
            st.error("فشل الاتصال بالمترجم، يرجى المحاولة مرة أخرى.")
else:
